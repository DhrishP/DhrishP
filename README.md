## Hi, I'm Dhrish 💭  

**AI-powered fullstack engineer** learning and building systems.  

“Ex-founding engineer who scaled to 265k+ users, building AI agents, LLM systems, backends, and frontends, and constantly refining taste.”


### Skills 🛠️
- **Languages:** TypeScript/JavaScript, Python, SQL  
- **AI/LLMs:** Vercel AI SDK, LangChain, LangSmith  
- **Backend:** Node.js, Express, Flask, FastAPI  
- **Frontend:** Next.js, React  
- **Infra:** AWS, Docker, MongoDB, PostgreSQL, Redis  
- **Tools:** Prisma, Drizzle, Mongoose  


### Work Experience 👔  

| Role | Company | Focus | Period |
|------|--------|------|--------|
| AI Full Stack Engineer | Induced | Agents, LLM Systems, Backend | 2025 — 2026 |
| Founding AI Fullstack Engineer | VeerOne | Tools, RSC, Fullstack  | 2024 — 2025 |
| Software Developer Intern | CerebralZip | Frontend, Backend | 2024 |


### Projects 🐾  

- **[todo-asap](https://t.me/todoAsap_bot)** — telegram bot based on Karpathy’s note-taking [method](https://karpathy.bearblog.dev/the-append-and-review-note/)  
- **[links](https://links.curiouslymotivated.com/)** — curated links + personal agent workflows  
- **[RAGVault](https://github.com/DhrishP/RAGVault)** — terminal-based local RAG system  
- **[twitter-finetune-3b](https://huggingface.co/whycurious101/twitter-finetune-3b)** — finetuned 3B model for viral tweets  
- **[voice-agents](https://github.com/DhrishP/voice-agents)** — production-style voice agent architecture  


### Blogs ✍️  

- **[Why AGI won’t be useful as you think](https://open.substack.com/pub/whycurious101/p/agi-wont-be-useful-as-you-think?r=3b560k&utm_campaign=post&utm_medium=web)** — thoughts on AGI being overhyped  
- **[Why Simple Things Scale](https://open.substack.com/pub/whycurious101/p/why-simple-things-scale)** — why simplification beats over-optimization  

\n

**tinkering, improving my taste, and figuring things out**  